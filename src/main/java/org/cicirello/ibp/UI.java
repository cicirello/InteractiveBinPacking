...
public class UI extends JPanel {
	...
	public UI(InteractiveBinPacking f) {
		...
		int[] weights = {36, 33, 39, 43, 7, 19, 37, 8, 29, 28, 37, 23, 29, 
			10, 22, 11, 33, 9, 17, 30};
		CallBack onSetInstance = new CallBack() {
			@Override
			public void call() {
				bottom.refresh();
				refresh();
		...
		CallBack executeRefresh = new CallBack() {
			@Override
			public void call() {
				refresh();
			}
		};
		state = new ApplicationState(9, new Floor(weights), onSetInstance, executeRefresh, executeRefresh);
