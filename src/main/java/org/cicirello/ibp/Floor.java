...
/**
 * This class represents the collection of items for a bin packing instance that are not yet in any
 * bin. It also generates random instances upon construction.
 *
...
public class Floor extends Bin {
   ...
   * random number generator, this constructor will regenerate an identical instance if none of the
   * parameters change.
   *
  ...
  public Floor(int sizeLow, int sizeHigh, int numItems, long seed) {
    ...
    int[] itemSizes =
        ApplicationState.createRandomItemSizes(sizeLow, sizeHigh, numItems, new Random(seed));
    init(itemSizes);
  }
  ...
  public Floor(int sizeLow, int sizeHigh, int numItems) {
    ...
    if (sizeHigh < sizeLow || numItems < 0) throw new IllegalArgumentException();
    int[] itemSizes =
        ApplicationState.createRandomItemSizes(sizeLow, sizeHigh, numItems, new Random());
    init(itemSizes);
  }
