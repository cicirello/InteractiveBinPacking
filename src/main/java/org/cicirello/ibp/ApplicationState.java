...
/**
 * This class is used to maintain the state of the application, including the state of the solving
 * of the current problem instance (e.g., it encapsulates the Bin objects, etc) as well as other
 * state info such as the mode (practice mode vs one of the heuristic modes). This includes the
 * logic needed to determine if a item/bin is consistent with a heuristic.
 *
...
final class ApplicationState {
   ...
   * @param onSetNewInstance The call method of this object will be called whenever a new instance
   *     is set.
  ...
  public ApplicationState(
      int numBins,
      Floor instance,
      CallBack onSetNewInstance,
      CallBack onSortFloor,
      CallBack onReset) {
  ...
  /**
   * Converts the mode int to the mode string (i.e., string to use to describe mode in dialog boxes,
   * etc).
   *
   * @param mode The mode as an integer (see class constants).
  ...
  /** Resets the instance, removing all elements from bins, and listing in original order. */
  public void reset() {
    theFloor.removeAll();
  ...
  /**
   * Records that a move was attempted but failed, which can be for a variety of reasons, such as
   * the destination doesn't have sufficient space, the item is already at the destination, or the
   * move violates the chosen heuristic.
   */
   ...
   *
   * @param decreasing if true, then the sort is from largest to smallest. otherwise, sorts from
   *     smallest to largest.
   */
  ...
  /**
   * Computes a lower bound on the number of bins required to pack all of the items. It may or may
   * not be possible to pack the items in this number of bins. However, it is impossible to use
   * fewer bins than this.
   *
   * @return a lower bound on the number of bins needed for this instance of bin packing.
   */
  ...
  /**
   * Retrieves and returns the session log formatted for in application viewing in html.
   *
